    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="flex items-center gap-3">
          <motion.div
            className="bg-gradient-to-r from-purple-500 to-blue-600 rounded-full p-3"
      ...
      <div className="flex flex-wrap gap-2">
        {categories.map((category) => (
          <Button
            ...
            className="flex items-center gap-2 text-lg py-2 px-4"
          >
            ...
            <div className="flex flex-wrap gap-3">
              {recentSymbolsData.map((symbol) => (
                <motion.button
                  ...
                  className={`bg-gradient-to-br ${symbol.color} text-white rounded-xl p-4 shadow-lg hover:shadow-xl transition-all duration-200 min-w-[120px]`}
                  whileHover={animationsEnabled ? { scale: 1.05 } : {}}
                ...
                >
                  ...
                  <div className="text-sm font-medium">{symbol.text}</div>
                </motion.button>
              ))}
            </div>
          </CardContent>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {filteredSymbols.map((symbol, index) => (
              <motion.button
                key={symbol.id}
                onClick={() => handleSymbolPress(symbol)}
                className={`bg-gradient-to-br ${symbol.color} text-white rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-200 aspect-square flex flex-col items-center justify-center`}
                whileHover={animationsEnabled ? { scale: 1.05 } : {}}
                whileTap={animationsEnabled ? { scale: 0.95 } : {}}
                initial={
              ...
              >
                <div className="text-4xl mb-3">{symbol.emoji}</div>
          ...
          <div className="space-y-3">
            {filteredSymbols.map((symbol, index) => (
              <motion.button
                key={symbol.id}
                onClick={() => handleSymbolPress(symbol)}
                className={`w-full bg-gradient-to-r ${symbol.color} text-white rounded-xl p-4 shadow-lg hover:shadow-xl transition-all duration-200 flex items-center gap-4`}
              ...
              >
                ...
                <div className="text-lg font-medium">{symbol.text}</div>
              </motion.button>
            ))}
          </div>
        )}
