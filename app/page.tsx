...
import { Calendar, Heart, MessageCircle, BookOpen, Flower2, Settings, Sparkles } from "lucide-react"
...
const navigationItems = [
  { id: "routines", label: "My Routines", icon: Calendar, color: "from-blue-400 to-blue-600" },
  { id: "emotions", label: "How I Feel", icon: Heart, color: "from-pink-400 to-pink-600" },
  { id: "communication", label: "Talk", icon: MessageCircle, color: "from-purple-400 to-purple-600" },
  { id: "stories", label: "Stories", icon: BookOpen, color: "from-green-400 to-green-600" },
  { id: "calm", label: "Calm Corner", icon: Flower2, color: "from-rose-400 to-rose-600" },
]

export default function MindNestApp() {
  const [currentPage, setCurrentPage] = useState("routines")
  const [showSettings, setShowSettings] = useState(false)
  const { theme, soundEnabled, animationsEnabled, updateSettings } = useAppStore()

  const renderPage = () => {
    if (showSettings) return <AppSettings onClose={() => setShowSettings(false)} />
  ...
  return (
    <div className={`min-h-screen ${getThemeClasses()}`}>
      ...
      <div className="relative z-10">
        {/* Header */}
        <header
          className={`${theme === "dark" ? "bg-gray-800/80" : theme === "high-contrast" ? "bg-black" : "bg-white/80"} backdrop-blur-sm border-b ${theme === "high-contrast" ? "border-yellow-400" : "border-gray-200"} sticky top-0 z-50`}
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-16">
              <div className="flex items-center space-x-3">
                ...
                <h1
                  className={`text-2xl font-bold ${theme === "high-contrast" ? "text-yellow-400" : "bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent"}`}
                >
                  MindNest
                </h1>
              </div>
              ...
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${theme === "high-contrast" ? "text-yellow-400 hover:bg-yellow-400 hover:text-black" : "text-gray-600 hover:text-gray-800 hover:bg-gray-100"}`}
                aria-label="Settings"
              >
        ...
        {!showSettings && (
          <nav
            className={`fixed bottom-0 left-0 right-0 ${theme === "dark" ? "bg-gray-800/95" : theme === "high-contrast" ? "bg-black" : "bg-white/95"} backdrop-blur-sm border-t ${theme === "high-contrast" ? "border-yellow-400" : "border-gray-200"} z-50`}
          >
